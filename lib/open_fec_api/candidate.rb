	...
	class Candidate < Client
  	...
  	### Methods: 'merge_options', 'make_request' are found in parent Client class
	    ...
	    def self.history_by_cycle(candidate_id, cycle)
	    	...
	    	make_request( self.get("/candidate/#{candidate_id}/history/#{cycle}/", query: {'api_key' => @@api_key } ))
	    end 
	    ...
	    def self.candidate_request(type, options = {})
	    	...
	    	return make_request( self.get(@@candidate_endpoints[type], query: query) )
	    end
	   	...
	   	def self.request_by_candidate_id(candidate_id, type, options = {})
    		...
    		make_request( self.get( uri, query: query ))
	    end 
